...
std::unique_ptr<JourneyModel> NavigationController::calculateDegreesAndDistanceToObject()
{
  ...
  auto vectorToRobotBack = calculateVectorToObject(robotBack_.get());
  double distanceInCm = distanceToObject * (ConfigController::getConfigInt("RobotLengthInMM") / std::sqrt(vectorToRobotBack.first * vectorToRobotBack.first + vectorToRobotBack.second * vectorToRobotBack.second));
...
std::pair<int, int> NavigationController::calculateVectorToObject(const CourseObject* courseObject) const
{
  ...
  const int middleRobotX = robotFront_->x1() - (robotFront_->x1() - robotBack_->x1());
  const int middleRobotY = robotFront_->y1() - (robotFront_->y1() - robotBack_->y1());
