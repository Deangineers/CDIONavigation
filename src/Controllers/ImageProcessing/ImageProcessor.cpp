...
void ImageProcessor::redPixelHelperFunction(const cv::Mat& frame, cv::Mat& mask)
{
  ...
  for (const auto& contour : contours)
  {
    ...
    // Approximate contour to polygon to reduce noise
    std::vector<cv::Point> approx;
    cv::approxPolyDP(contour, approx, 10.0, true); // epsilon = 10 can be tuned

    for (size_t i = 0; i < approx.size(); ++i)
    {
      cv::Point p1 = approx[i];
      cv::Point p2 = approx[(i + 1) % approx.size()]; // Close the polygon

      // Create a vector and check length
      ...
      // Prepare vector with start position
      auto vectorToCreate = std::make_unique<VectorWithStartPos>(p1.x, p1.y, vector);

      // Optionally check for wall validity
      if (!wallProcessor_->isWallValid(vectorToCreate.get()))
      {
        // continue; // Uncomment if you want to skip invalid walls
      }

      // Visualize vector on frame
      cv::line(frame, p1, p2, cv::Scalar(255, 0, 0),
               ConfigController::getConfigInt("WallWidth"), cv::LINE_AA);
      cv::putText(frame, std::to_string(label++), p1,
                  cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 255, 0), 2);

      // Register vector as blocked object
